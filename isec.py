...
def light_status(intersect_number, light_label):
    """Returns true if light is on, false if light is off
    ...
    """
    _check_init()
    return _intersect_object.get_light(intersect_number, light_label)
...
class Intersection(object):
    ...
    def get_light(self, intersect_number, light_label):
        """Return the state of the specified light"""
        self._check_intersect(intersect_number)
	self._check_light(light_label)
	return (self._light_state[intersect_number-1] & self._light_dict_on[light_label]) > 0
